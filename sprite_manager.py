...
class Block:
    ...
    Tracks the original sprite position so it can be rendered correctly
    even after rotation or locking.
    """
    
    def __init__(self, shape_type, sprite_dx, sprite_dy, sprite_surface=None):
        """
        Initialize a block.
    ...
    def copy(self):
        ...
        return Block(self.shape_type, self.sprite_dx, self.sprite_dy, self.sprite)
    
    def __repr__(self):
        return f"Block({self.shape_type}, {self.sprite_dx}, {self.sprite_dy})"
        ...
        for dy in range(height_blocks):
            for dx in range(width_blocks):
                ...
                try:
                    ...
                    # Store the block sprite
                    self.block_sprites[(shape_type, dx, dy)] = block_surface
                    
                except Exception as e:
                    print(f"Error splitting sprite at ({dx}, {dy}) for {shape_type}: {e}")
    
    def get_block_sprite(self, shape_type, dx, dy):
        """
        Get a pre-split 48x48 sprite for a specific block position.
        ...
        Returns:
            A 48x48 Surface with the block sprite, or None
        """
        if not self.use_sprites:
            return None
        
        return self.block_sprites.get((shape_type, dx, dy))
    
    def create_block(self, shape_type, dx, dy):
        """
        ...
        """
        sprite = self.get_block_sprite(shape_type, dx, dy)
        return Block(shape_type, dx, dy, sprite)
